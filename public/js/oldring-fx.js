...
(function () {
    ...
    function detectSpots(img, iw, ih) {
        ...
        ctx.drawImage(img, (SCAN - iw * s) / 2, (SCAN - ih * s) / 2, iw * s, ih * s);
        ...
        var spots = picked.map(function (p) {
            ...
            return Math.hypot(sp.x - iw / 2, sp.y - ih / 2) <= maxRad;
        });

        return { spots: spots, text: detectTextBand(d, SCAN, iw, ih) };
    }
    ...
    function detectTextBand(d, S, iw, ih) {
        if (!d) return null;
        ...
        for (var ry = 0; ry < S; ry++) {
            ...
            var w = (xMax - xMin + 1) * (iw / S);
            if (w < iw * 0.5) continue;   // must span most of the ring
            best = { x: xMin * (iw / S), y: y0 * (ih / S), w: w, h: h };
            break;
        }
        return best;
    }
