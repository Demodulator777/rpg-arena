...
function renderCharacter() {
    ...
    // Equipment slot tiles — emoji + name on hover with full stats tooltip
    const eqSlots=[
    ...
    const eqGrid=eqSlots.map(({slot,icon,label})=>{
        const item=eq[slot];
        if (!item) return `
        ...
        return `
            <div class="eq-slot filled" style="border-color:${qc}44" title="${tipText}">
                <span class="eq-slot-icon" style="font-size:1.6rem">${item.emoji||icon}</span>
...
async function doStartMission(zoneId, spotId, missionIdx, size='small') {
    ...
    try {
        ...
        character=await api('GET','/game/character'); // refresh to get new MP
        renderTopBar();
        closeMissionModal2();
    ...
    if (invTab==='equipment') {
        ...
        <div class="inv-equipment-grid">${gear.map(i=>{
            ...
            return '<div class="inv-item-cell '+(eq?'inv-item-equipped ':'')+qc+'" onmouseenter="showItemTooltip(event,'+i.id+')" onmouseleave="scheduleHideTooltip()" onclick="toggleEquipItem('+i.id+',\''+d.slot+'\','+(eq?'true':'false')+')">'
                +'<div class="inv-item-icon">'+(d.emoji||'📦')+'</div>'
...
function renderShop() {
    ...
    el.innerHTML=filtered.map(item=>{
        ...
        const effectHtml=item.effect?(()=>{
            ...
            <div class="shop-card-header"><span class="shop-card-icon">${item.emoji}</span><span class="shop-card-name">${item.name}</span><span class="shop-card-tier">Lv.${item.level||1}</span></div>
...
async function openProfile(id) {
    ...
    try {
        ...
        const eqHtml=slots.map(([slot,fallback])=>{
            ...
            return `<div title="${item.name}\n${statsText}" style="width:44px;height:44px;border-radius:8px;background:rgba(255,255,255,0.06);border:1px solid ${qc}33;display:flex;align-items:center;justify-content:center;font-size:1.4rem;cursor:default;position:relative" data-tip="${escHtml(item.name)}">${item.emoji||fallback}</div>`;
        }).join('');
