...
function buildLeaderboardRow(p, fallbackRank = 1, extraClass = '') {
    ...
    const squadHtml = p.squad_logo && p.squad_id
        ? `<span ${actionAttrs('showSquadDetail', p.squad_id)} style="cursor:pointer;display:inline-flex;align-items:center;gap:4px;flex-shrink:0;margin-left:6px"><img src="${escHtml(p.squad_logo)}" alt="" style="width:28px;height:28px;border-radius:50%;object-fit:cover;border:none"><span style="font-size:0.8rem;color:var(--gold)">${escHtml(p.squad_name||'')}</span></span>`
        : '';
