...
function applyServerUI() {
  const s = getServer();
  const label = document.getElementById('server-select-label');
...
function openServerPicker() {
  const m = document.getElementById('server-picker-modal');
...
function selectServerFromPicker(id) {
  closeServerPicker();
  const target = GAME_SERVERS.find(x => x.id === id);
  if (target && target.id !== getServerId()) window.location.href = target.url;
}
function returnToBeta() {
  window.location.href = 'https://battle-online.com/';
}
(function initServerSwitch() {
  function wire() {
    ...
    // two subdomains, so redirecting "back to your chosen server" ping-pongs forever.
    // The current server is decided purely by hostname and shown via applyServerUI().
    applyServerUI();
  }
        ...
        if (!res.ok) {
            ...
            if (res.status === 401 && errMsg === 'Session expired') {
                if (!window.__forcedLogoutShown) {
                    ...
                    logout();
                    openGameNoticeDialog({ title: 'Logged Out', message: 'Your account was logged in from another device/browser.', confirmLabel: 'OK' });
                }
...
async function loadMissions() {
    ...
    try {
        // Async overlay refresh in background — corrects display if cached data was stale
...
async function renderAutoCompletePanel(zoneId, spotId) {
    ...
    qpBtns.forEach(btn => {
        btn.addEventListener('click', (ev) => {
            ev.stopPropagation();
            const invId = String(btn.getAttribute('data-inv'));
            const dir = Number(btn.getAttribute('data-dir'));
            const p = potions.find(x => String(x.inventoryId) === invId);
            if (!p) return;
            const max = status?.maxPotions ?? 10;
            const cur = _autoSelectedPotions.get(invId) || 0;
            const next = cur + dir;
            if (next <= 0) {
                _autoSelectedPotions.delete(invId);
            } else if (next > Number(p.qty)) {
                return;
            } else if (dir > 0 && autoPotionCount(potions) >= max) {
                return;
            } else {
                _autoSelectedPotions.set(invId, Math.min(next, max));
            }
...
function updateAutoPoolTotal(panel, potions) {
    ...
    for (const p of potions) {
        const q = _autoSelectedPotions.get(String(p.inventoryId)) || 0;
        if (q > 0) total += p.mp * q;
    }
...
function showTabAlert(msg) {
    ...
    if (!ov) {
        ...
        ov.querySelector('[data-action="closeForgeAlert"]').addEventListener('click', () => ov.classList.add('hidden'));
            ...
            if (squads.length === 0) {
                ...
                squads.forEach((s, i) => {
                    ...
                    const logoHtml = s.logo
                        ? `<img src="${escHtml(s.logo)}" alt="" style="width:36px;height:36px;border-radius:50%;object-fit:cover;flex-shrink:0">`
                        : `<div style="width:36px;height:36px;border-radius:50%;flex-shrink:0;background:rgba(255,255,255,0.04);display:flex;align-items:center;justify-content:center;font-size:1rem">🛡️</div>`;
                    html += `<div class="lb-row" ${actionAttrs('showSquadDetail', s.squad_id)}>
                        <div class="lb-rank ${rc}">${rs}</div>
                        ${logoHtml}
...
async function openProfile(id) {
    ...
    try {
        ...
        const beastBonus = p.beast_stat_bonus || {};
        const totalStr = str + (itemBonus.strength || 0) + (setBonus.strength || 0) + (beastBonus.str || 0);
        const totalDef = def + (itemBonus.defense || 0) + (setBonus.defense || 0) + (beastBonus.def || 0);
        const baseDmgMin = Math.floor(totalStr * 0.5);
        const baseDmgMax = baseDmgMin + 4;
        const gearDmgMin = Object.values(eq).reduce((sum, item) => sum + (item?.stats?.dmg_min || 0) + (item?.wp_stats?.dmg_min || 0), 0);
        const gearDmgMax = Object.values(eq).reduce((sum, item) => sum + (item?.stats?.dmg_max || 0) + (item?.wp_stats?.dmg_max || 0), 0);
...
function alignAvatarPop(pop, attempt) {
    ...
    // RIGHT side only: narrows to 13%-87% and ends at 30% height so wide gear
    // (shields/weapons) tucks behind the frame's right ring. LEFT side keeps the
    // original wide 9% down to 45%. The diagonal between the two cuts is
    // invisible — pop and base are the same rendered image below it.
    const xL = (0.09 * W + ox).toFixed(2), xR2 = (0.87 * W + ox).toFixed(2);
    const yTop = (oy - E).toFixed(2), y0 = oy.toFixed(2);
    const yCutR = (oy + 0.30 * H).toFixed(2), yCutL = (oy + 0.45 * H).toFixed(2);
    pop.style.clipPath = `polygon(${ox.toFixed(2)}px ${yTop}px, ${(W + ox).toFixed(2)}px ${yTop}px, ` +
        `${(W + ox).toFixed(2)}px ${y0}px, ${xR2}px ${y0}px, ${xR2}px ${yCutR}px, ` +
        `${xL}px ${yCutL}px, ${xL}px ${y0}px, ${ox.toFixed(2)}px ${y0}px)`;
...
// Pan the big character-preview image via transform:translate (NOT object-position).
// object-position can't move an image that exactly fills an axis (e.g. a full-height
// portrait), so translate guarantees real movement in every direction and can exceed
// the frame bounds ("over the limit"). Formula is shared by the editor + render sites.
function avatarTransformCSS(x, y, z) {
    const W = 230, H = 300;
    const px = (x != null && isFinite(x)) ? Number(x) : 50;
    const py = (y != null && isFinite(y)) ? Number(y) : 50;
    const tx = ((px - 50) / 100) * W;
    const ty = ((py - 50) / 100) * H;
    return `translate(${tx}px, ${ty}px) scale(${avatarZoomVal(z)})`;
}

// Renders the big character-tab avatar. Uses object-fit:contain with the image
// panned via transform:translate (so it moves in every direction — even past the
// frame bounds — which object-position cannot do for images that fill an axis) and
// zoomed via transform:scale. Kept inside an overflow-hidden frame. A single fit
// mode makes zoom continuous: 100% = whole image, 110% = genuine 10% zoom.
function avatarImgHtml(src, off) {
    const x = (off && off.x != null) ? off.x : 50;
    const y = (off && off.y != null) ? off.y : 50;
    return `<div class="eq-avatar-zoom"><img class="eq-avatar-custom" src="${escHtml(src)}" alt="avatar" data-error-opacity-zero="true" style="object-position:50% 50%;transform:${avatarTransformCSS(x, y, off)}"></div>`;
}

// Standalone Character Avatar Editor — edits ONLY the big character-preview crop
// (position + zoom over the 230x300 frame). Independent of the round blob.
async function openCharacterAvatarEditor() {
    const existing = document.getElementById('char-avatar-editor');
    if (existing) existing.remove();

    const src = profilePicSrc(character.profile_pic || character.class + '.png');
    const off = character.char_pic_offset || character.profile_pic_offset || { x: 50, y: 50, z: 1 };
    let cX = (off.x != null) ? off.x : 50;
    let cY = (off.y != null) ? off.y : 50;
    let cZ = avatarZoomVal(off);

    const overlay = document.createElement('div');
    overlay.id = 'char-avatar-editor';
    ...
    overlay.innerHTML = `
        <div style="background:linear-gradient(135deg,#1a1230,#161625);border-radius:12px;padding:24px;max-width:92%;max-height:92%;overflow:auto;text-align:center;">
            ...
            <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
                ...
                <button class="btn-primary" id="ced-close">Close</button>
            </div>
            <div id="ced-msg" style="margin-top:12px;font-size:12px;color:rgba(255,255,255,0.6);"></div>
        </div>`;
    document.body.appendChild(overlay);

    const preview = overlay.querySelector('#ced-preview');
    ...
    const startDrag = (startEvt) => {
        ...
        if (preview.setPointerCapture) { try { preview.setPointerCapture(startEvt.pointerId); } catch {} }
    };
    preview.addEventListener('pointerdown', startDrag);

    const zoomInput = overlay.querySelector('#ced-zoom');
    const zoomVal = overlay.querySelector('#ced-zoom-val');
    zoomInput.addEventListener('input', () => {
        cZ = Number(zoomInput.value);
    ...
    overlay.querySelector('#ced-close').addEventListener('click', () => overlay.remove());
    overlay.querySelector('#ced-save').addEventListener('click', async () => {
        const msg = overlay.querySelector('#ced-msg');
        try {
            const res = await api('POST', '/game/profile-pic/char-offset', { x: cX, y: cY, z: cZ });
            if (character) character.char_pic_offset = { x: cX, y: cY, z: cZ };
            renderCharacter();
            renderTopBar();
            if (msg) { msg.textContent = 'Saved!'; msg.style.color = 'var(--green, #22c55e)'; }
        } catch (e) {
            if (msg) { msg.textContent = e.message; msg.style.color = 'var(--red, #ef4444)'; }
        }
    });
}
...
const currentPic = character.profile_pic || `${character.class}.png`;
        ...
        const blobOff = character.profile_pic_offset || { x: 50, y: 50 };
        const curX = (blobOff.x != null) ? blobOff.x : 50;
        const curY = (blobOff.y != null) ? blobOff.y : 50;
        const adjusterHtml = `
            <div style="border-top:1px solid rgba(255,255,255,0.15);margin-top:16px;padding-top:12px;width:100%;">
                <div style="font-size:12px;color:#e0b84c;margin-bottom:8px;">🎯 Round Avatar Position</div>
                ...
                <div style="font-size:10px;color:rgba(255,255,255,0.5);margin-top:8px;">Drag to move your avatar within the circle. This only affects your round (blob) avatar — the big character-preview crop is edited separately.</div>
            </div>`;
...
async function uploadProfilePic(e) {
    ...
    try {
        ...
        const prevOff = (character && character.char_pic_offset) || (character && character.profile_pic_offset) || {};
        const prevX = (prevOff.x != null) ? prevOff.x : 50;
        const prevY = (prevOff.y != null) ? prevOff.y : 50;
        const prevZ = avatarZoomVal(prevOff);
        const previewUrl = URL.createObjectURL(finalFile);
        const prevImgStyle = `object-fit:contain;object-position:50% 50%;transform:${avatarTransformCSS(prevX, prevY, prevZ)};transform-origin:center;`;
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:2147483647;display:flex;align-items:center;justify-content:center;';
        ...
        overlay.innerHTML = `
            <div style="background:linear-gradient(135deg,#1a1230,#161625);border-radius:12px;padding:24px;max-width:92%;max-height:92%;overflow:auto;text-align:center;">
                ...
                <div id="pic-preview-frame" style="width:280px;height:340px;margin:0 auto 16px auto;background:linear-gradient(180deg,#151a30,#0d0d1a);border:1px solid rgba(255,255,255,0.15);border-radius:8px;display:flex;align-items:center;justify-content:center;">
                    <img src="${previewUrl}" style="width:230px;height:300px;${prevImgStyle}" data-error-hide="true">
                </div>
