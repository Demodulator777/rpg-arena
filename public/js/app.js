...
(function initServerSwitch() {
  function wireServerSelect() {
    applyServerUI();
    const sel = document.getElementById('server-select');
    if (!sel || sel.dataset._wired) return;
    sel.dataset._wired = '1';
    // Simple: the dropdown value maps to a server URL. Changing it goes there.
    // No auto-redirect on load (localStorage is per-origin -> would ping-pong).
    sel.addEventListener('change', () => {
      const t = GAME_SERVERS.find(x => x.id === sel.value);
      if (t && t.id !== getServerId()) window.location.href = t.url;
    });
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', wireServerSelect);
  else wireServerSelect();
})();
