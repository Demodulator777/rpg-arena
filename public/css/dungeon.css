...
.dfp-banner img[src]:not([src^=""]) { z-index: 1; }
.dfp-floor-badge {
  position: absolute;
  top: 4px;
  left: 4px;
...
/* Multi-enemy combat styles - add these */
.combat-monster-entry {
  transition: background 0.15s ease;
}

.combat-monster-entry.current-target {
  background: rgba(124, 58, 237, 0.2) !important;
  border-left: 3px solid var(--dtheme, #7c3aed);
}

.combat-monster-entry.defeated {
  opacity: 0.5;
  filter: grayscale(0.5);
}

.combat-monster-entry .fighter-hp-bar-wrap {
  background: rgba(0, 0, 0, 0.3);
}
...
@media (max-width: 768px) {
  ...
  /* Hide inline log on mobile to save space */
  .dungeon-hud-log-inline {
    display: none;
  }
