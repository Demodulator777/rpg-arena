...
@media (max-width: 640px) {
    ...
    /* FIX: Ensure tab content area is visible and scrollable */
    .game-body {
        display: flex;
        flex-direction: column;
        flex: 1;
...
.class-scene-mage { --class-scene-tint: rgba(78, 145, 255, 0.25); }
.class-scene-warrior { --class-scene-tint: rgba(224, 119, 72, 0.24); }
.class-scene-paladin { --class-scene-tint: rgba(245, 207, 108, 0.24); }
.class-scene-rogue { --class-scene-tint: rgba(101, 222, 173, 0.22); }
...
@media (max-width: 680px) {
    ...
    .char-panel-record { grid-column: auto; }
    .element-strip-row { align-items: flex-start; }
}
...
.char-panel-record { grid-column: 2; }
.char-panel h3 {
...
.stat-row {
    display: grid;
    grid-template-columns: 24px 108px minmax(120px, 1fr) 64px 40px;
    align-items: center;
    column-gap: 10px;
    padding: 8px 0;
...
.crit-fill {
    ...
    box-shadow: 0 0 12px rgba(201,146,42,0.18);
}

@media (max-width: 920px) {
    .stat-row {
        grid-template-columns: 24px 96px minmax(90px, 1fr) 58px 36px;
    }
}
...
.inv-equipment-grid { display: grid; grid-template-columns: repeat(auto-fill,minmax(90px,1fr)); gap: 10px; }
.inv-item-cell {
    position: relative; background: rgba(255,255,255,0.03);
    border: 1px solid var(--border); border-radius: 10px;
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    gap: 5px; cursor: pointer; padding: 10px 6px; aspect-ratio: 1 / 1.1;
...
.inv-item-icon img { width: 64px; height: 64px; object-fit: contain; display: block; }
...
.item-tooltip {
    position: fixed; z-index: 2000; width: 220px;
...
.tt-actions { display: flex; gap: 5px; border-top: 1px solid rgba(255,255,255,0.08); padding: 10px 14px; }
...
.shop-card-icon img { width: 68px; height: 68px; object-fit: contain; border-radius: 8px; display: block; }
...
/* ════════════════════════════════════════════════════════════════════════
   INBOX
   ════════════════════════════════════════════════════════════════════════ */
.msg-row { padding: 12px 16px; border-bottom: 1px solid var(--border); cursor: pointer; transition: background 0.12s; }
...
.bug-report-intro p { margin: 0 0 5px 0; font-size: 0.85rem; }
.bug-report-note { font-size: 0.75rem; color: var(--text-dim); margin-top: 5px; }
...
@media (max-width: 640px) {
    ...
    .game-logo { width: 130%; margin-left: -15%; margin-right: -15%; border-radius: 16px; }
...
.upgrade-item-current, .upgrade-item-next {
    font-size: 0.8rem;
    color: var(--text-dim);
    margin-top: 4px;
}
...
.upgrade-component-card:hover {
    background: rgba(255,255,255,0.08);
    border-color: var(--border-mid);
}

.upgrade-component-card.selected {
    background: rgba(155,89,182,0.15);
    border-color: #9b59b6;
}
