...
(async () => {
  ...
  await db.execute({ sql: 'INSERT OR REPLACE INTO server_settings (key, value) VALUES (?, ?)', args: ['spirit_beast_enabled', 'true'] });
  const r2 = await db.execute('SELECT * FROM server_settings');
  console.log('AFTER WRITE true:', JSON.stringify(r2.rows));
  await db.execute({ sql: 'INSERT OR REPLACE INTO server_settings (key, value) VALUES (?, ?)', args: ['spirit_beast_enabled', 'false'] });
