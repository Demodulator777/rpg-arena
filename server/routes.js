/* ════════════════════════════════════════════════════════════════════════
   ...
   Cinematic · Immersive · No external image dependencies
   ════════════════════════════════════════════════════════════════════════ */

@import url('https://fonts.googleapis.com/css2?family=Cinzel+Decorative:wght@700;900&family=Cinzel:wght@400;600;700&family=IM+Fell+English:ital@0;1&family=Inter:wght@300;400;500;600;700&display=swap');
...
/* Logo image (auth screen) — full-width bleed like original */
.game-logo {
    position: relative;
    z-index: 10;
    width: 200%;
...
/* Background battle image behind login container */
...
.class-card:hover { border-color: var(--gold-dim); transform: translateY(-2px); background: rgba(255,255,255,0.05); }
.class-card.selected { border-color: var(--gold); background: rgba(212,168,67,0.08); box-shadow: 0 0 24px rgba(212,168,67,0.2); }
.class-icon { font-size: 2.2rem; margin-bottom: 8px; }
...
.class-desc { font-family: 'IM Fell English', serif; font-style: italic; font-size: 0.82rem; color: var(--text); }
...
.btn-upgrade {
    ...
    background: linear-gradient(145deg, #2a1800, #6b3d00, var(--gold));
    border: 1px solid rgba(201,146,42,0.32);
    border-radius: var(--radius-sm);
    color: #1a0800; font-size: 0.8rem; font-weight: 700;
    cursor: pointer; transition: all 0.18s;
    box-shadow: 0 2px 10px rgba(201,146,42,0.1), inset 0 1px 0 rgba(255,255,255,0.12);
}
.btn-upgrade:hover:not(:disabled) { opacity: 0.9; transform: translateY(-1px); }
.btn-upgrade:disabled { background: rgba(255,255,255,0.04); color: var(--text-dim); border-color: var(--border); cursor: not-allowed; transform: none; }
...
/* top-bar.png decorative overlay — sits on top, pointer-events off so it doesn't block clicks */
.topbar::after {
    content: '';
...
/* Keep all topbar children above the overlay */
...
.topbar-avatar-wrap {
    position: relative; display: flex; align-items: center; gap: 11px; flex-shrink: 0;
}
.topbar-avatar {
    width: 46px; height: 46px; border-radius: 50%; object-fit: cover;
    border: 2px solid rgba(201,146,42,0.5);
    box-shadow: 0 0 0 1px rgba(201,146,42,0.15), 0 0 18px rgba(201,146,42,0.2);
    background: var(--bg3);
    transition: box-shadow 0.3s;
}
.topbar-avatar:hover { box-shadow: 0 0 0 2px rgba(201,146,42,0.6), 0 0 28px rgba(201,146,42,0.4); }
.topbar-avatar-info { display: flex; flex-direction: column; gap: 3px; }
.topbar-name {
    font-family: 'Cinzel', serif; font-size: 0.88rem; font-weight: 700;
    color: var(--text-bright); white-space: nowrap; letter-spacing: 0.02em;
...
/* Resource bars */
.topbar-bars { display: flex; flex-direction: column; gap: 4px; flex: 1; min-width: 0; max-width: 380px; }
.topbar-bar-row { display: flex; align-items: center; gap: 7px; }
.topbar-bar-label {
    font-size: 0.56rem; font-weight: 800; color: var(--text-dim);
    letter-spacing: 0.1em; width: 16px; flex-shrink: 0; text-transform: uppercase;
}
.topbar-bar-track {
...
.topbar-bar-fill::after {
    ...
    background: rgba(255,255,255,0.18); border-radius: 3px 3px 0 0;
}
.topbar-bar-fill.hp-fill  { background: linear-gradient(90deg, #155a30, #1e8449, #2ecc71); }
.topbar-bar-fill.xp-fill  { background: linear-gradient(90deg, #1a4a7a, #2874a6, #3498db); }
.topbar-bar-fill.mp-fill  { background: linear-gradient(90deg, #4a0070, #7d3c98, #9b59b6); }
.topbar-bar-text {
    font-size: 0.6rem; color: var(--text-dim); white-space: nowrap; min-width: 62px; text-align: right;
}
...
.topbar-logout {
    font-size: 0.7rem; padding: 6px 13px; border-radius: var(--radius-sm);
    border: 1px solid var(--border-mid);
    background: rgba(255,255,255,0.03);
    color: var(--text-dim); cursor: pointer; transition: all 0.15s; white-space: nowrap;
}
.topbar-logout:hover { background: rgba(224,82,82,0.1); color: var(--red-light); border-color: rgba(224,82,82,0.3); }
...
/* ── Sidebar (base styles — mobile uses these as bottom nav) ── */
.sidebar {
    width: var(--sidebar-w); min-width: var(--sidebar-w);
    background: linear-gradient(180deg, var(--bg2) 0%, rgba(8,13,23,0.98) 100%);
...
.sidebar::after {
    ...
    background: linear-gradient(180deg, transparent, rgba(201,146,42,0.12) 30%, rgba(201,146,42,0.08) 70%, transparent);
    pointer-events: none;
}
...
.sidebar-section-label::after {
    ...
    background: linear-gradient(90deg, rgba(201,146,42,0.2), transparent);
}
...
@media (min-width: 641px) {
    ...
    .sidebar {
        ...
        background: linear-gradient(180deg, rgba(10,15,26,0.98), rgba(8,12,20,0.98));
        flex-shrink: 0;
    }
    .sidebar::-webkit-scrollbar { display: none; }
    .sidebar::after { display: none; }
    /* Hide section labels entirely on desktop top nav */
    .sidebar-section-label {
        display: none;
    }
    /* Compact horizontal buttons — all same fixed width */
    .nav-btn {
    ...
    /* Active state: subtle bottom border */
    .nav-btn.active { color: #7dd3f8; box-shadow: inset 0 -2px 0 #3498db; }
    .nav-btn.active::before { opacity: 1; }
    /* Edge elements match button height */
    .nav-edge { height: 32px !important; }
    /* Content fills remaining height */
    .tab-content-area { flex: 1; min-height: 0; overflow-y: auto; }
}
...
.nav-btn {
    ...
    transition: color 0.14s, filter 0.14s, transform 0.1s;
    position: relative;
    z-index: 0;
    overflow: visible;
}
/* Image on pseudo — flush by default */
.nav-btn::before {
    content: '';
    position: absolute;
    inset: 0;
...
/* Edge-only decorative elements — left and right caps */
.nav-edge {
    flex-shrink: 0;
    width: 4px;
    height: 32px;
    background-image: url('/images/logo/button.png');
    /* Show image at natural proportional height, wide enough to have edges visible */
...
@media (min-width: 641px) {
    .nav-edge { display: block; }
    /* Left edge: anchor image to left, clip to 4px — shows the left ornamental tip */
    .nav-edge-left  { background-position: left center; }
    /* Right edge: anchor image to right, clip to 4px — shows the right ornamental tip */
    .nav-edge-right { background-position: right center; }
}
.nav-btn:hover { color: #fff; transform: translateX(2px); }
.nav-btn:hover::before { opacity: 1; }
.nav-btn.active { color: #7dd3f8; font-weight: 700; }
.nav-btn.active::before { opacity: 1; }
...
.unread-badge {
    ...
    box-shadow: 0 0 8px rgba(224,82,82,0.5);
}
...
.tab-content-area {
    ...
    background:
        ...
        url('/images/logo/arena.png') center center / cover no-repeat fixed;
    position: relative;
}
...
@media (max-width: 680px) { .char-grid { grid-template-columns: 1fr; } }
...
.stat-label { flex: 1; color: var(--text-dim); font-size: 0.8rem; }
.stat-val { font-size: 1rem; color: var(--text-bright); font-weight: 700; min-width: 28px; text-align: right; }
.stat-bar-wrap { width: 72px; }
...
.str-fill  { background: linear-gradient(90deg, #8b1a1a, #e74c3c); }
.def-fill  { background: linear-gradient(90deg, #1a4a7a, #3498db); }
.agi-fill  { background: linear-gradient(90deg, #7a6000, #f1c40f); }
.mag-fill  { background: linear-gradient(90deg, #5a1a7a, #9b59b6); }
.vit-fill  { background: linear-gradient(90deg, #0e5c2e, #2ecc71); }
.hit-fill  { background: linear-gradient(90deg, #1a5a5a, #1abc9c); }
.crit-fill { background: linear-gradient(90deg, #7a3000, #e67e22); }
...
.eq-slot {
    ...
    min-height: 76px; cursor: default; position: relative;
...
.eq-slot.filled:hover { background: rgba(255,255,255,0.07); transform: translateY(-1px); }
.eq-slot-icon { font-size: 1.6rem; line-height: 1; }
.eq-slot-label {
    font-size: 0.58rem; color: rgba(255,255,255,0.38); text-align: center;
...
.upgrade-card:hover { border-color: var(--border-mid); }
.upgrade-card-header { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
.upgrade-card-icon { font-size: 1.4rem; }
.upgrade-card-name { font-size: 0.92rem; color: var(--gold-light); font-weight: 600; }
.upgrade-card-val { margin-left: auto; font-size: 1.35rem; color: var(--text-bright); font-weight: 700; }
.upgrade-discount { font-size: 0.68rem; color: var(--green); margin-bottom: 9px; }
.upgrade-cost { font-size: 0.78rem; color: var(--text-dim); margin-bottom: 12px; }
.upgrade-cost strong { color: var(--gold); }
...
.forge-tier { font-size:0.65rem; color:var(--text-dim); background:rgba(255,255,255,0.05); border-radius:4px; padding:2px 7px; }
.forge-desc { font-size:0.72rem; color:var(--text-dim); font-style:italic; margin-bottom:6px; line-height:1.45; }
...
.inv-equipment-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(76px,1fr)); gap:9px; }
.inv-item-cell {
    ...
    display:flex; flex-direction:column; align-items:center; justify-content:center;
    gap:4px; cursor:pointer; padding:6px 4px;
...
.inv-item-icon { font-size:1.85rem; line-height:1; }
...
.inv-item-equipped-dot { position:absolute; top:4px; right:4px; width:6px; height:6px; background:var(--gold); border-radius:50%; box-shadow:0 0 5px rgba(201,146,42,0.8); }
...
/* Item tooltip */
.item-tooltip {
    position:fixed; z-index:2000; width:244px;
    background: linear-gradient(145deg, rgba(8,12,22,0.98), rgba(14,20,34,0.98));
    border:1px solid var(--border-mid);
    border-radius:var(--radius); padding:14px;
    box-shadow:0 16px 50px rgba(0,0,0,0.85), 0 0 0 1px rgba(201,146,42,0.08);
    pointer-events:auto; backdrop-filter:blur(10px);
}
.item-tooltip.hidden { display:none; }
.tt-header { display:flex; align-items:center; gap:10px; margin-bottom:8px; }
.tt-icon { font-size:1.75rem; flex-shrink:0; }
.tt-name { font-size:0.86rem; font-weight:600; line-height:1.2; }
.tt-meta { font-size:0.66rem; color:var(--text-dim); margin-top:2px; }
.tt-desc { font-size:0.69rem; color:var(--text-dim); margin-bottom:8px; font-style:italic; line-height:1.5; }
...
.tt-actions { display:flex; gap:5px; margin-top:8px; border-top:1px solid var(--border); padding-top:8px; }
.tt-btn { flex:1; padding:6px 4px; border-radius:6px; font-size:0.7rem; font-weight:600; cursor:pointer; border:1px solid; transition:background 0.12s; }
...
.shop-card.legendary {
    background: linear-gradient(145deg, var(--bg2), #1e1200);
    border-color:rgba(255,215,0,0.35);
    box-shadow:0 0 24px rgba(255,200,0,0.08);
}
.shop-card.rare {
    background: linear-gradient(145deg, var(--bg2), #0e0e24);
...
.shop-card-icon { font-size:1.9rem; }
...
.shop-card-stats {
    ...
    padding:9px; border-radius:var(--radius-sm); margin-bottom:10px; font-size:0.76rem;
}
...
.shop-card-footer { display:flex; justify-content:space-between; align-items:center; margin-top:10px; }
...
.lb-row:hover { border-color:rgba(201,146,42,0.25); background: linear-gradient(90deg, rgba(201,146,42,0.04), rgba(16,25,41,0.9)); transform: translateX(2px); }
.lb-rank { width:34px; text-align:center; font-size:0.84rem; color:var(--text-dim); flex-shrink:0; font-family:'Cinzel',serif; }
.lb-rank.gold-rank   { color:#ffd700; font-weight:700; text-shadow:0 0 10px rgba(255,215,0,0.4); }
.lb-rank.silver-rank { color:#c0c0c0; font-weight:700; }
.lb-rank.bronze-rank { color:#cd7f32; font-weight:700; }
.lb-info { flex:1; min-width:0; }
.lb-name { font-size:0.9rem; color:var(--text-bright); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.lb-sub { font-size:0.68rem; color:var(--text-dim); font-style:italic; }
.lb-stats { display:flex; gap:14px; align-items:center; flex-shrink:0; }
.lb-stat { text-align:center; min-width:50px; }
.lb-stat-val { font-size:0.9rem; font-weight:600; }
.lb-stat-lbl { font-size:0.55rem; color:var(--text-dim); letter-spacing:0.07em; text-transform:uppercase; }
...
.msg-row.unread { background:rgba(52,143,209,0.04); border-left:3px solid var(--gold); }
.msg-header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:3px; }
.msg-from { font-size:0.8rem; font-weight:600; color:var(--text); }
.msg-from.unread-from { color:var(--blue); }
.msg-date { font-size:0.66rem; color:var(--text-dim); }
.msg-subject { font-size:0.78rem; color:var(--text-dim); }
.msg-body-full { font-size:0.76rem; color:rgba(255,255,255,0.6); margin-top:8px; padding:10px; background:rgba(0,0,0,0.25); border-radius:var(--radius-sm); white-space:pre-wrap; }
.msg-actions { gap:8px; margin-top:10px; }
...
.battle-result-box { max-width:580px; width:100%; }
.battle-outcome {
...
/* Tab must fill full height so absolute overlay covers everything */
#tab-missions {
    position: relative;
    min-height: 100%;
}

.mission-overlay-inner,
.travel-overlay-inner {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 14px;
...
.mission-overlay-progress-track {
    width: 100%; height: 6px;
    background: rgba(255,255,255,0.07);
    border-radius: 3px; overflow: hidden;
}
.mission-overlay-progress-fill {
...
.mission-modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.85); backdrop-filter:blur(5px); z-index:1000; display:flex; align-items:stretch; justify-content:center; }
...
.mz-travel-btn:disabled { opacity:0.42; cursor:not-allowed; }
...
.mz-spot-card.mz-spot-locked{ opacity:0.42; cursor:not-allowed; }
...
.mz-mission-card{ background:rgba(255,255,255,0.025); border:1px solid var(--border); border-radius:var(--radius); overflow:hidden; cursor:pointer; transition: all 0.15s; }
...
.zone-select{
    ...
    color:#fff; font-size:0.76rem; cursor:pointer;
...
.msg-bar{
    ...
    color:var(--text-bright); margin-top:10px;
    transition: all 0.3s;
}
...
.filter-btn{
    ...
    color:var(--text-dim); cursor:pointer; font-size:0.76rem;
    transition: all 0.14s;
}
.filter-btn:hover{ background:rgba(255,255,255,0.05); color:rgba(255,255,255,0.85); border-color:var(--border-mid); }
.filter-btn.active{
    background: rgba(52,143,209,0.12);
    border-color:rgba(52,143,209,0.32); color:var(--blue); font-weight:600;
}
...
@media(max-width:640px){
    .sidebar{
        ...
        scrollbar-width:none;
        border-right:none; border-top:1px solid rgba(201,146,42,0.2);
        background: rgba(5,8,14,0.97);
        padding:5px 6px; z-index:200; gap:2px;
        flex-wrap:nowrap;
    }
    .sidebar::-webkit-scrollbar { display:none; }
    .sidebar::after { display:none; }
    .sidebar-section-label{ display:none; }
    /* Hide edge decorations on mobile */
    .nav-edge { display:none !important; }
    .nav-btn{
        flex-direction:column; gap:2px; font-size:0;
        padding:7px 6px;
        min-width:52px; width:52px !important; height:auto !important; min-height:unset !important;
        flex-shrink:0;
        text-align:center; justify-content:center; border-radius:8px;
    }
    /* Icons via ::after — nth-child(1) is nav-edge-left (hidden), so buttons start at 2 */
    .nav-btn::after{ font-size:1.3rem; line-height:1; }
    .nav-btn:nth-of-type(1)::after  { content:'👤'; }
    .nav-btn:nth-of-type(2)::after  { content:'⚔️'; }
    .nav-btn:nth-of-type(3)::after  { content:'✨'; }
    .nav-btn:nth-of-type(4)::after  { content:'💪'; }
    .nav-btn:nth-of-type(5)::after  { content:'⬆️'; }
    .nav-btn:nth-of-type(6)::after  { content:'🗺️'; }
    .nav-btn:nth-of-type(7)::after  { content:'🔨'; }
    .nav-btn:nth-of-type(8)::after  { content:'🎒'; }
    .nav-btn:nth-of-type(9)::after  { content:'🛒'; }
    .nav-btn:nth-of-type(10)::after { content:'🏆'; }
    .nav-btn:nth-of-type(11)::after { content:'✉️'; }
    .nav-btn .unread-badge{ font-size:0.55rem; padding:1px 4px; min-width:14px; }
    .nav-btn.active::before{ display:none; }
    .nav-btn.active{ background:rgba(52,143,209,0.18); }
    .tab-content-area{ padding: 14px; padding-bottom: 70px; }
    /* On mobile the bottom nav is fixed so we need fixed here to cover it */
    .mission-overlay,
    .travel-overlay {
        position: fixed;
        inset: 0;
    ...
    .topbar {
        ...
        overflow: visible; justify-content: flex-start; flex-wrap: nowrap;
    }
    ...
    .topbar-bar-track { min-width: 0; height: 8px; padding: 0; }
    .topbar-bar-label { font-size: 0.52rem; width: 14px; }
    .topbar-bar-text { display: none; }
    .topbar-right {
        width: 100%; flex: none; display: flex; flex-direction: row;
    ...
    .topbar-logout { font-size: 0.65rem; padding: 4px 10px; flex-shrink: 0; margin-left: auto; }
    .char-hero-content{ flex-direction:column; text-align:center; padding:18px; gap:14px; }
    .char-avatar-ring{ width:88px; height:88px; }
    .char-hero-name{ font-size:1.45rem; }
}

@media(max-width:500px){
    .char-hero-content{ padding:16px; gap:12px; }
