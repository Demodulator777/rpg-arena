...
router.use(async (req, res, next) => {
    ...
    res.on('finish', async () => {
        try {
            ...
            // tab_viewed only matters for mission endpoints (generous 24h window)
            const isMissionPath = path.includes('/missions/start') || path.includes('/missions/collect') || path.includes('/missions/tab-viewed');
            const tabViewed = isMissionPath ? ((now - lastTab) < 86400 ? 1 : 0) : 1;
            let bodyStr = '';
...
ACHIEVEMENTS.push(
    ...
    {
        id: 'weekly_damage_750k',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Skilled',
        desc: 'Deal 750,000 damage in a single week.',
        icon: '⚡',
        metric: 'total_dmg_dealt',
        target: 750000,
        rewards: { gold: 75000, consumable: { id: 'elixir_might', qty: 2 } },
    },
    {
        id: 'weekly_damage_1m',
        chain: 'weekly_damage',
    ...
    {
        id: 'weekly_damage_1_5m',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Veteran',
        desc: 'Deal 1,500,000 damage in a single week.',
        icon: '🔥',
        metric: 'total_dmg_dealt',
        target: 1500000,
        rewards: { gold: 125000, gems: 2 },
    },
    {
        id: 'weekly_damage_2m',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Elite',
        desc: 'Deal 2,000,000 damage in a single week.',
        icon: '🔥',
        metric: 'total_dmg_dealt',
        target: 2000000,
        rewards: { gold: 150000, gems: 3 },
    },
    {
    ...
    },
    {
        ...
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Champion',
        desc: 'Deal 3,000,000 damage in a single week.',
        icon: '☄️',
        metric: 'total_dmg_dealt',
        target: 3000000,
        rewards: { gems: 6, lootbox: { id: 'lootbox_epic', qty: 1 } },
    },
    {
        id: 'weekly_damage_4m',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Hero',
        desc: 'Deal 4,000,000 damage in a single week.',
        icon: '☄️',
        metric: 'total_dmg_dealt',
        target: 4000000,
        rewards: { gems: 8, lootbox: { id: 'lootbox_epic', qty: 1 } },
    },
    {
        id: 'weekly_damage_5m',
        chain: 'weekly_damage',
    ...
    },
    {
        ...
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Warlord',
        desc: 'Deal 6,000,000 damage in a single week.',
        icon: '🌟',
        metric: 'total_dmg_dealt',
        target: 6000000,
        rewards: { gems: 12, lootbox: { id: 'lootbox_legendary', qty: 1 } },
    },
    {
        id: 'weekly_damage_7m',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Conqueror',
        desc: 'Deal 7,000,000 damage in a single week.',
        icon: '🌟',
        metric: 'total_dmg_dealt',
        target: 7000000,
        rewards: { gems: 14, lootbox: { id: 'lootbox_legendary', qty: 1 } },
    },
    {
        id: 'weekly_damage_8m',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Destroyer',
        desc: 'Deal 8,000,000 damage in a single week.',
        icon: '🌟',
        metric: 'total_dmg_dealt',
        target: 8000000,
        rewards: { gems: 16, lootbox: { id: 'lootbox_legendary', qty: 1 } },
    },
    {
        id: 'weekly_damage_9m',
        chain: 'weekly_damage',
        category: 'combat',
        name: 'Weekly Damage: Annihilator',
        desc: 'Deal 9,000,000 damage in a single week.',
        icon: '🌟',
        metric: 'total_dmg_dealt',
        target: 9000000,
        rewards: { gems: 18, lootbox: { id: 'lootbox_legendary', qty: 1 } },
    },
    {
...
const NPC_BASE_CAPTURE = {
    main:   { min_members: 50, npc_count: 50, npc_level: 50 },
    large:  { min_members: 40, npc_count: 40, npc_level: 40 },
    medium: { min_members: 30, npc_count: 30, npc_level: 30 },
    small:  { min_members: 20, npc_count: 20, npc_level: 20 },
};
...
router.post('/squads/bases/:baseId/capture', auth, async (req, res) => {
    try {
        const db = await getDb();
        const char = await getCurrentCharacter(db, req.user.userId);
        if (!char) return res.status(404).json({ error: 'No character' });
        ...
        if (!membership) return res.status(403).json({ error: 'Only leaders, co-leaders and officers can capture bases.' });
        const baseId = Number(req.params.baseId);
        const base = await dbGet(db, 'SELECT * FROM clan_bases WHERE id=?', [baseId]);
        if (!base) return res.status(404).json({ error: 'Base not found.' });
        if (base.owner_squad_id) return res.status(400).json({ error: 'Base is already occupied.' });
        const cfg = NPC_BASE_CAPTURE[base.tier];
        if (!cfg) return res.status(400).json({ error: 'Invalid base tier.' });
        // Check member count
        const memberCount = await dbGet(db, 'SELECT COUNT(*) AS c FROM squad_members WHERE squad_id=?', [membership.squad_id]);
        if (Number(memberCount?.c || 0) < cfg.min_members) {
        ...
        // Check for existing war or owned base
        const attackerOwned = await dbGet(db, 'SELECT 1 FROM clan_bases WHERE owner_squad_id=? LIMIT 1', [membership.squad_id]);
        if (attackerOwned) return res.status(400).json({ error: 'Your squad already owns a base.' });
        ...
        const npcsPerOutpost = Math.ceil(cfg.npc_count / 5);
        const npcPower = calcNpcPower(cfg.npc_level);
        for (let i = 0; i < 5; i++) {
...
function calcNpcPower(npcLevel) {
    const stat = npcLevel * 3;
    return npcLevel * 10 + stat * 5;
}
...
router.get('/squads/wars/active', auth, async (req, res) => {
    try {
        ...
        const result = wars.map(w => ({
            ...
            attacker_name: w.attacker_name, defender_name: w.defender_name || 'NPC Defenders',
            ...
            is_npc_war: Number(w.is_npc_war || 0) === 1,
...
router.get('/squads/wars/:warId', auth, async (req, res) => {
    try {
        ...
        let npcInfo = null;
        if (isNpcWar) {
            const cfg = NPC_BASE_CAPTURE[war.base_tier];
            if (cfg) npcInfo = { npc_count: cfg.npc_count, npc_level: cfg.npc_level, npc_power: calcNpcPower(cfg.npc_level) };
        } else {
...
const DUNGEON_MONSTER_POOL = [
    ...
    { id:'shadow_stalker', name:'Shadow Stalker', icon:'🐺', image:'/images/dungeon/monsters/shadow_stalker.jpg', hp:400, atk:55, def:25, steal:true,  minFloor:10, isMiniBoss:true, lore:'The Shadow Stalker is a tough dog-like creature that attacks from the shadows.' },
    { id:'crystal_golem',  name:'Crystal Golem',  icon:'💎', image:'/images/dungeon/monsters/crystal_golem.jpg', hp:600, atk:40, def:45, steal:false, minFloor:15, isMiniBoss:true },
    { id:'flame_revenant', name:'Flame Revenant', icon:'🔥', image:'/images/dungeon/monsters/flame_revenant.jpg', hp:350, atk:70, def:20, steal:false, minFloor:20, isMiniBoss:true },
    { id:'frost_wyrmling', name:'Frost Wyrmling', icon:'❄️', image:'/images/dungeon/monsters/frost_wyrmling.jpg', hp:450, atk:60, def:30, steal:true,  minFloor:25, isMiniBoss:true },
    { id:'void_stalker',   name:'Void Stalker',   icon:'🌑', image:'/images/dungeon/monsters/void_stalker.jpg', hp:500, atk:75, def:28, steal:true,  minFloor:30, isMiniBoss:true },
    { id:'doom_knight',    name:'Doom Knight',    icon:'⚔️', image:'/images/dungeon/monsters/doom_knight.jpg', hp:700, atk:65, def:50, steal:true,  minFloor:35, isMiniBoss:true },
];

const DUNGEON_MINI_BOSS_POOL = [
    { id:'shadow_stalker', name:'Shadow Stalker', icon:'🐺', baseHp:400, baseAtk:55, baseDef:25, minFloor:5,  image:'/images/dungeon/monsters/shadow_stalker.jpg', lore:'The Shadow Stalker is a tough dog-like creature that attacks from the shadows.' },
    { id:'crystal_golem',  name:'Crystal Golem',  icon:'💎', baseHp:600, baseAtk:40, baseDef:45, minFloor:15, image:'/images/dungeon/monsters/crystal_golem.jpg' },
    { id:'flame_revenant', name:'Flame Revenant', icon:'🔥', baseHp:350, baseAtk:70, baseDef:20, minFloor:20, image:'/images/dungeon/monsters/flame_revenant.jpg' },
    { id:'frost_wyrmling', name:'Frost Wyrmling', icon:'❄️', baseHp:450, baseAtk:60, baseDef:30, minFloor:25, image:'/images/dungeon/monsters/frost_wyrmling.jpg' },
    { id:'void_stalker',   name:'Void Stalker',   icon:'🌑', baseHp:500, baseAtk:75, baseDef:28, minFloor:30, image:'/images/dungeon/monsters/void_stalker.jpg' },
    { id:'doom_knight',    name:'Doom Knight',    icon:'⚔️', baseHp:700, baseAtk:65, baseDef:50, minFloor:35, image:'/images/dungeon/monsters/doom_knight.jpg' },
];
